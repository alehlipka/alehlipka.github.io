...
Для поддержки произвольного доступа сжатые текстуры обычно организуют в блоки одного размера. Basis Universal всегда использует блоки 4×4 пикселя.
...
| Уровень Mip | Ширина × Высота, px | Ширина × Высота, блоки |
|:-:|-:|-:|
| 0 | 100 x 200 | 25 x 50 |
| 1 | 50 x 100 | 13 x 25 |
| 2 | 25 x 50 | 7 x 13 |
...
Здесь базовый уровень mip полностью заполняет сжатые блоки размером `25 × 50`. У следующих уровней часть пикселей в блоках остаётся заполненной дополнением (padding). Это дополнение (если оно присутствует) не влияет на обертывание текстурных координат и недоступно при сэмплинге.
...
- Если платформа не поддерживает ни один из перечисленных форматов, данные ETC1S можно декодировать в [несжатый RGBA](#uncompressed-formats).
...
Текстуры UASTC состоят из блоков 4×4, каждый блок занимает ровно 16 байт. Сжатие Zstandard (если оно присутствует) следует декодировать до транскодирования текстуры.
...
[ASTC 4×4](#astc-4x4) следует выбирать по умолчанию, при наличии поддержки ASTC LDR.
...
Иногда декодирование UASTC в [16-битные упакованные форматы](#16-bit-packed-formats-1) (RGB565 или RGBA4444) может давать лучший результат, чем транскодирование в ETC, BC1/BC3 или PVRTC1, ценой увеличения (примерно в 2 раза) объёма памяти GPU.
...
Транскодированные данные занимают 16 байт на каждый блок 4×4.
...
Транскодированные данные занимают 8 байт на каждый блок 4×4.
